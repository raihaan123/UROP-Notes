...
def train():
    with wandb.init() as run:
        ...
        net = dde.nn.FNN(layer_size, activation, initializer, dropout_rate=0.1)
        ...
        def input_transform(t):
            return tf.concat(
                (
                    ...
                    tf.cos(t),
                    tf.sin(t),
                    t*0 + tf.pi                     # Curious to see what happens when we add a physical constant as an input feature
                    # tf.sin(2 * t),
                    # tf.sin(3 * t),
                    # tf.sin(4 * t)
                ),
