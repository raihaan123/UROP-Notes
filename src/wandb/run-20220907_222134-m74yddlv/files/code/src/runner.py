...
# TODO: Add parameter mu as an input
# time_domain = dde.geometry.TimeDomain(0, t_f)
# mu_domain = dde.geometry.Interval(0, 2)
...
def train():
    with wandb.init() as run:
        ...
        net = dde.nn.FNN(layer_size, activation, initializer, "L2", dropout_rate=0.1)
        ...
        def input_transform(t):
            return tf.concat(
                (
                    ...
                    tf.cos(t),
                    tf.sin(t),
                    t*0 + np.pi,                     # Curious to see what happens when we add a physical constant as an input feature
                    t*0 + mu                         # And also the ODE parameter
